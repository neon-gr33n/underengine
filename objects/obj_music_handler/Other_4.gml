if room!=rm_battle fname=(global.room_data[$ "ROOMS"][$ global.room_data[$ "__ROOM__"][$ room_get_name(room)]][$ "THEME"]==undefined ? fname : global.room_data[$ "ROOMS"][$ global.room_data[$ "__ROOM__"][$ room_get_name(room)]][$ "THEME"])
else fname=noone;
if fname==noone mus_stop(global.currentmus[0])
else {
	if(room!=rm_battle&&!paused){
