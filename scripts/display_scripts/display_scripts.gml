...
function set_resolution(_design_value, _is_design_value_vertical = false, _is_orientation_vertical = false, _resize_all_rooms = false, _scale = undefined, _overide_other_value = undefined) {
	...
	if _is_orientation_vertical 
	{
		...
		_aspect_ratio	= 3/4;//_real_width >= _real_height ? _real_height / _real_width : _real_width / _real_height;
	...
	if _resize_all_rooms //apply resolution results to all rooms?
	{
		for (var i = 0; i < room_last; i++) 
		{
			if room_exists(i)
			{
				room_set_width(i, global._display_width);
				room_set_height(i, global._display_height);
			}
		}
	}
...
function set_display_sizes() {
	global._display_width = global.resolutions[global.resolution_index].w
	global._display_height = global.resolutions[global.resolution_index].h
	
	global._windowed_width = global._display_width*(!global.asp_ratio ? 1 : 1.5);
	global._windowed_height = global._display_width*(!global.asp_ratio ? 3/4 : 1.5*9/16);
}
